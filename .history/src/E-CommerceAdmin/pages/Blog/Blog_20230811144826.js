...
const Blog = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const [anonymousMode, setAnonymousMode] = useState(null);
    const [doublecoins, setDoubleCoins] = useState(null);

    const payload = { name , price , crushAlert , firstLetter , fullName , perWeek , totalWeek , anonymousMode , dob }

    const postHandler = async (e) => {
      e.preventDefault();
      try {
        const res = await axios.post("https://smoke-backend.vercel.app/api/v1/subsription/createSubscription" , );
      } catch (e) {
