...
const Dashboard = () => {
  ...
  const fetchAllUsers = async () => {
    await axios
      .get("https://smoke-backend.vercel.app/api/v1/admin/User/allUser", {
  ...
  const fetchDashboardCardsData = async ()=>{
    await axios.get('https://smoke-backend.vercel.app/api/v1/admin/dashboard',{
      headers: { Authorization: `Bearer ${adminToken}` },
    }).then(res => setDashData(res.data.data));
  } 

  // Dashboard Graph Data Fethcing 
  const fetchDashboardGraph = async ()=>{
    await axios.get('https://smoke-backend.vercel.app/api/v1/admin/dashboardGraph',{
      headers: { Authorization: `Bearer ${adminToken}` },
    }).then(res =>console.log("graph data "+ res.data.data));
  }

  useEffect(() => {
    // For Fetching and Filtering all Active Users
    fetchAllUsers();
  ...
  const card = [
    ...
    },
    {
      progress: "bg-green-400",
      title: "Active User",
      number: users.filter(user => user.status === 'Active').length,
